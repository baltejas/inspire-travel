...
class cluster {
    ...
    provision() {
      console.log("PROVISIONNING CLUSTER");
...
//            .then(this._queryOnline.bind(this))
//            .then(this._buildPrimaryIndex.bind(this))
//            .then(this._buildFtsIndex.bind(this))
        ...
        return new Promise(
            (resolve, reject) => {
                request.get({
                    ...
                    if (err) {
                        console.log("COUCHBASE INSTANCE AT " + locals.endPoint + " NOT FOUND.");
                    ...
                    for (var i = 0; i < body.length; i++) {
                        if (body[i].name == locals.sampleBucket) {
                            console.log("Cannot provision an already built cluster.");
    ...
    _buildPrimaryIndex(){
      console.log("-- BUILD PRIMARY INDEX");
      return new Promise(
          (resolve, reject) => {
              request.get({
                  url: "http://" + this.endPointQuery + "/query?statement=CREATE+PRIMARY+INDEX+ON+restful-sample+USING+GSI",
                  auth: {
                      'user': config.couchbase.user,
                      'pass': config.couchbase.password,
                      'sendImmediately': true
    ...
    _verifyNodejsVersion(locals) {
      console.log("-- VERIFY NODE VERSION");
        return new Promise(
            (resolve, reject)=> {
